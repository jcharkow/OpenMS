...
namespace OpenMS
{
  ...
  void MRMDecoy::generateDecoys(const OpenMS::TargetedExperimentTwo& exp, OpenMS::TargetedExperimentTwo& dec,
                                ...
                                const bool enable_specific_losses, const bool enable_unspecific_losses, const int round_decPow) const
  {
    ...
    MRMDecoy::TransitionVectorTypeTwo decoy_transitions;
    for (Size i = 0; i < exp.getProteins().size(); i++)
    {
      OpenMS::TargetedExperimentTwo::Protein protein = exp.getProteins()[i];
    ...
    {
      OpenMS::TargetedExperimentTwo::Peptide peptide = exp.getPeptides()[pep_idx];
    ...
    MRMDecoy::PeptideTransitionMapTypeTwo peptide_trans_map;
    ...
    for (MRMDecoy::PeptideTransitionMapTypeTwo::iterator pep_it = peptide_trans_map.begin();
         pep_it != peptide_trans_map.end(); ++pep_it)
    {
      ...
      {
        const ReactionMonitoringTransitionTwo tr = *(pep_it->second[i]);

        if (!tr.isDetectingTransition() || tr.getDecoyTransitionType() == ReactionMonitoringTransitionTwo::DECOY)
        {
          continue;
        }

        ReactionMonitoringTransitionTwo decoy_tr = tr; // copy the target transition

        decoy_tr.setNativeID(decoy_tag + tr.getNativeID());
        decoy_tr.setDecoyTransitionType(ReactionMonitoringTransitionTwo::DECOY);
    ...
    decoy_transitions.erase(std::remove_if(
                            ...
                            [&exclusion_peptides](const OpenMS::ReactionMonitoringTransitionTwo& tr)
    {
