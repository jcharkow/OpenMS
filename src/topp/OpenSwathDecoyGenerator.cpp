...
protected:
  ...
  ExitCodes main_(int, const char **) override
  {
    ...
    TargetedExperimentTwo targeted_mergedTwo;
    {
      TargetedExperiment targeted_exp;
      TargetedExperiment targeted_decoy;
      TargetedExperimentTwo targeted_expTwo;
      TargetedExperimentTwo targeted_decoyTwo;

      // Load data
      OPENMS_LOG_INFO << "Loading targets from file: " << in << std::endl;
      if (in_type == FileTypes::TSV || in_type == FileTypes::MRM)
      {
        // if .tsv use targeted_expTwo as it is more memory efficient
        ...
        tsv_reader.convertTSVToTargetedExperiment(tr_file, in_type, targeted_expTwo, batch_size);
        tsv_reader.validateTargetedExperiment(targeted_expTwo);
      }
      ...
      if (targeted_exp.getTransitions().empty())
      {
        ...
        OPENMS_LOG_INFO << "Number of target peptides: " << targeted_expTwo.getPeptides().size() << std::endl;
        OPENMS_LOG_INFO << "Number of decoy peptides: " << targeted_decoyTwo.getPeptides().size() << std::endl;
        OPENMS_LOG_INFO << "Number of target proteins: " << targeted_expTwo.getProteins().size() << std::endl;
        OPENMS_LOG_INFO << "Number of decoy proteins: " << targeted_decoyTwo.getProteins().size() << std::endl;

        if ((float)targeted_decoyTwo.getPeptides().size() / (float)targeted_expTwo.getPeptides().size() < min_decoy_fraction || (float)targeted_decoyTwo.getProteins().size() / (float)targeted_expTwo.getProteins().size() < min_decoy_fraction)
        {
        ...
        if (separate)
        {
          ...
          targeted_mergedTwo = std::move(targeted_decoyTwo);
        }
        else
        {
          OPENMS_LOG_INFO << "Writing targets and decoys to file: " << out << std::endl;
          targeted_mergedTwo = std::move(targeted_expTwo);
          targeted_mergedTwo += std::move(targeted_decoyTwo);
        }
